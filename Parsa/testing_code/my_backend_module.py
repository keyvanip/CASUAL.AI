...
def extract_city_from_message(message: str) -> str:
    # Look for a capitalized word or two capitalized words next to each other (e.g., New York, Paris)
    city_candidates = re.findall(r'\b(?:[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\b', message)
    if city_candidates:
        # Take the first reasonable candidate
        return city_candidates[-1]  # Most recent city mention
...
def load_user_metadata(user):
    path = f"frontend/stylist-app/public/data/{user.lower()}_metadata.json"
